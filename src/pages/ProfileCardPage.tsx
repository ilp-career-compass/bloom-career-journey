...
export default function ProfileCardPage({ studentIdOverride, readOnly }: ProfileCardPageProps) {
      ...
      if (cachedRows) {
        for (const row of cachedRows) {
          if (row.assessment_type === 'career_direction') {
            ...
            keyMap[row.assessment_type] = Array.isArray(row.keywords) ? row.keywords : [];
          }
        }
      }
      setKeywords(keyMap);

      // Fetch summary template questions for preview on incomplete cards
      const tplMap: Record<string, string[]> = {};
      for (const mod of MODULES) {
        try {
          const { data: tplData } = await supabase.rpc('get_summary_template', {
            p_assessment_type: mod.assessmentType
          });
          if (tplData && tplData.length > 0) {
            const tpl = tplData[0].summary_questions;
            const langBlock = tpl?.[lang] || tpl?.en;
            if (langBlock) {
              const qs: string[] = [];
              for (let i = 1; i <= 16; i++) {
                const q = langBlock[`question${i}`];
                if (q && typeof q === 'string') qs.push(q);
              }
              if (qs.length > 0) tplMap[mod.key] = qs.slice(0, 4);
            }
          }
        } catch {
  ...
  useEffect(() => {
    ...
    for (const mod of MODULES) {
      ...
      if (summary && summary.approval_status === 'approved' && !keywords[mod.key]?.length) {
        regenerateKeywords(mod.key, summary);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, summaries]);

  const regenerateKeywords = async (assessmentType: string, summary: AssessmentSummary) => {
    ...
    try {
      ...
      if (result.success && result.keywords) {
        setKeywords(prev => ({ ...prev, [assessmentType]: result.keywords! }));
      ...
      logger.error('Keyword generation failed for', assessmentType, err);
    } finally {
  ...
  const generateDirection = async () => {
    ...
    try {
      const dreamsKw = keywords['dreams'] || [];
      const hobbiesKw = keywords['hobbies'] || [];
      const roleModelsKw = keywords['role_models'] || [];
      const result = await aiSummaryService.generateCareerDirection(
        dreamsKw, hobbiesKw, roleModelsKw, studentName, lang
      );
      ...
      <div className="bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-700 text-white">
        <div className="container mx-auto px-4 py-10">
          ...
          <div className="text-center">
            ...
            <h1 className="text-2xl md:text-3xl font-bold mb-1">{PAGE_TITLE[lang]}</h1>
            ...
            <div className="mt-5 max-w-xs mx-auto">
              <div className="flex items-center justify-between text-xs text-indigo-200 mb-1.5">
                <span>{SECTIONS_COMPLETE[lang](completedCount)}</span>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {MODULES.map(mod => {
            ...
            const kws = keywords[mod.key] || [];
            const isGenerating = generatingKeys.has(mod.key);
            const previewQuestions = templateQuestions[mod.key] || [];
            const IconComp = mod.icon;

            return (
              <Card
                key={mod.key}
                className="rounded-xl shadow-md hover:shadow-lg border border-gray-100 bg-white transition-all duration-200 overflow-hidden"
              >
                {/* Colored top strip */}
                      ...
                      <ul className="space-y-2">
                        {kws.map((kw, i) => (
                          <li key={i} className="flex items-start gap-2 text-sm">
                            <span className={`mt-1.5 h-2 w-2 rounded-full shrink-0 ${mod.dotColor}`} />
                      ...
                      {previewQuestions.length > 0 && (
                        <ul className="space-y-1.5 mb-3">
                          {previewQuestions.map((q, i) => (
                            <li key={i} className="text-xs text-gray-400 leading-snug truncate">
                              • {q}
                            </li>
                          ))}
                        </ul>
