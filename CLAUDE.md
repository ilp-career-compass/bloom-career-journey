...
| **roadmap-audit** | Career Roadmap 22-point gap analysis + full fix pass (May 2026). **High** — G1: autosave race: `debouncedSaveRef` pattern + `finally`-block reschedule ensures in-flight save never silently drops subsequent keystrokes; G2: failed-save rows re-queued into `pendingSavesRef` without overwriting newer user edits; G3: student INSERT/UPDATE RLS restricted to editable milestones (`beginning_9th`, `midterm_9th`, `end_9th`) only (migration `20260506000003`). **Medium** — G4: `MILESTONES`, `COLUMN_LABELS`, `RoadmapRow`, `getMilestoneLabel` extracted to `src/utils/roadmapConfig.ts` — single source of truth for both pages; G5: CLAUDE.md corrected to 8 milestones / 5 locked; G6/G7: `TeacherStudentRoadmapPage` uses teacher's `useLang` for page title + column headers; milestone labels keep student language; G8: `animate-pulse` removed from highlight (ring + bg-blue-50 remains); G10: locked-row placeholder changed from "Available later" to `—`; G11: amber prompt + Continue button gated behind `highlightedIsEditable` — locked milestones never show fill-prompt when deep-linked; G12: `roadmapRedirectedRef` (Set) in `StudentDashboard` prevents redirect loop (fires once per assessment type per session); G13: teacher view gains error + empty-state ("Student hasn't filled in their career plans yet"); G14: student view gains error state + translated Retry button; G22: teacher SELECT policies on `career_roadmap` and `profile_card_cache` rebuilt on `teacher_id` JOIN (migration `20260506000004`) — students with `class_id = NULL` now visible; also drops the broken `20260402000001` career_roadmap policy that compared `career_roadmap.student_id` (`users.id`) against `students.id` (always false). **Low** — G15: `NOT VALID CHECK (char_length ≤ 500)` on `plan_a/b/c` + `maxLength={500}` on textarea (migration `20260506000005`); G16: `created_at` column added; `updated_at` fetched + shown in teacher header as locale-formatted date; G18: `navigate(-1)` guarded with `window.history.state?.idx > 0` + `/student`/`/teacher` fallback for direct links; G19: `dbMilestonesRef` tracks existing DB rows — phantom empty upserts skipped when user types-then-clears a field. |
